...
### Two-stem separation model

如果你想训练一个drums和bass分离模型,你需要:

1. 在 `conf/config.yaml` 中修改 `sources` 为 `['drums', 'bass']`
2. 相应地修改 `weights` 为 `[1., 1.]`
3. 准备数据集:
   - 每个音频样本需要包含 `drums.wav` 和 `bass.wav` 两个文件
   - 如果使用MusDB数据集,可以直接使用,因为它本身就包含这两个音轨
   - 如果使用自定义数据集,确保正确分离出鼓点和贝斯音轨
4. 其他训练参数和流程保持不变
